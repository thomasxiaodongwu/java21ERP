...
import cn.xiaodong.infra.module.pay.dal.mysql.demo.PayDemoTransferMapper;
...
public class PayDemoTransferServiceImpl implements PayDemoTransferService {
    ...
    private void validatePayeeInfo(Integer transferType, Map<String, String> payeeInfo) {
        PayTransferTypeEnum transferTypeEnum = PayTransferTypeEnum.typeOf(transferType);
        switch (transferTypeEnum) {
            case ALIPAY_BALANCE: {
                if (StrUtil.isEmpty(MapUtil.getStr(payeeInfo, PayTransferTypeEnum.ALIPAY_LOGON_ID))) {
                    throw ServiceExceptionUtil.exception(ErrorCodeConstants.PAY_TRANSFER_ALIPAY_LOGIN_ID_IS_EMPTY);
                }
                if (StrUtil.isEmpty(MapUtil.getStr(payeeInfo, PayTransferTypeEnum.ALIPAY_ACCOUNT_NAME))) {
